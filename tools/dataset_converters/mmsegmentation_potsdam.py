...
def parse_args():
    ...
    parser.add_argument('dataset_path', help='')
...
def clip_big_image(image_path, clip_save_dir, args, to_label=False):
    ...
    # For example, given one 6000 x 6000 original image, the clip size is
    # 512 and stride size is 256, thus it would generate many 512x512 tiles.
    ...
    num_rows = math.ceil((h - clip_size) / stride_size) if math.ceil(
        (h - clip_size) / stride_size) * stride_size + clip_size >= h else math.ceil(
            (h - clip_size) / stride_size) + 1
    num_cols = math.ceil((w - clip_size) / stride_size) if math.ceil(
        (w - clip_size) / stride_size) * stride_size + clip_size >= w else math.ceil(
    ...
    if to_label:
        # Convert RGB label to class index (0-6)
    ...
    for box in boxes:
        ...
        clipped_image = image[start_y:end_y, start_x:end_x] if to_label else image[
